...
\subsection{Monte Carlo Audit Results}
\label{subsec:mc_evaluation}

We run constraint-consistent Monte Carlo sampling on all 298 eliminations to quantify Popularity Dissonance probabilities. The mean $P(\text{Wrongful})$ is 0.490 and the median is 0.529; at a 5\% threshold, 17 cases are classified as Definite-Correct and the remaining 281 are Uncertain.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/mc_probability_distribution.pdf}
    \caption{\textbf{Distribution of Popularity Dissonance Probabilities.} Histogram + KDE show the overall fairness landscape with a median near 0.53.}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Evolution Across Seasons.} Mean $P(\text{Wrongful})$ with 95\% CI reveals higher dissonance in rank-rule eras (S1--S2, S28+).}
...
\begin{figure}[H]
    ...
    \caption{\textbf{Top Cases with Confidence Intervals.} The most likely dissonant eliminations include several judge-save era weeks.}
...
At 5,000 samples per elimination, the full Monte Carlo audit completes in a few minutes on a standard laptop; per-episode LP solves remain sub-second. This enables \textbf{pre-broadcast consistency checking} before results air.
